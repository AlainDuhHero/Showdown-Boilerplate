...
function formatHTML(message) {
	...
	for (let i = 0; i < message.length; i++) {
		...
		total += '<span style = "color: ' + getColor() + '">' + Tools.escapeHTML(message[i]) + '</span>';
	}
	return total;
}
...
function nightclubify(message) {
	...
	return '|raw|<div style = "font-size: 11pt; margin: -3px; padding: 1px; background: black; color: white; text-shadow: 0px 0px 10px, 0px 0px 10px, 0px 0px 10px;">' +
		...
		'<button name = "parseCommand" value = "/user ' + user + '" style = "outline: none; cursor: pointer; text-shadow: inherit; background: none; padding: 0px; border: none; display: inline-block; color: ' + hashColor(toId(user)) + ';"><b>' + user + ':</b></button>' +
