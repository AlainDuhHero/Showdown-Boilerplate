...
let emotes = {
	...
	'feelsnii': 'http://i.imgur.com/IdrP4MY.gif',
...
function parseEmoticons(message, room, user, pm) {
	...
	if (room.battle || room.isPersonal) {
		if ((typeof message !== 'string' || room.disableEmoticons) && isDev(user.userid) && !user.can('upperstaff')) return false;
	} else {
		if ((typeof message !== 'string' || (!pm && room.chatRoomData.disableEmoticons)) && isDev(user.userid) && !user.can('upperstaff')) return false;
	}
