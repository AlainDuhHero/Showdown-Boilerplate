...
var Panagram = (function () {
	...
	Panagram.prototype.guess = function (user, guess) {
		if (guess.species === this.answer.species) {
			...
			Db('money').set(toId(user.userid), Db('credits').get(toId(user), 0) + 1).get(toId(user.userid));
			this.end();
		} else {
...
exports.commands = {
	...
	panagram: function (target, room, user, connection, cmd) {
		...
		if (room.id !== 'marketplace') return this.sendReply('|html|You can only start a game of Panagram in the <button name = "send" value = "/join marketplace">panagrams</button>');
