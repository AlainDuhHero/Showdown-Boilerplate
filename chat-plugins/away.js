...
exports.commands = {
	...
	back: function (target, room, user) {
		...
		if (statusIdx < 0) {
			user.isAway = false;
			if (user.can('lock', null, room)) this.add("|raw|-- " + nameColor(user.userid, true) + " is no longer away.");
			return false;
		}
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('lock', null, room)) this.add("|raw|-- " + nameColor(user.userid) + "'><strong>" + " is no longer " + status.toLowerCase() + ".");
	},
